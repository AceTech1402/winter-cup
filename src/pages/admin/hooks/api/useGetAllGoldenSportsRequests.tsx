...
const useGetAllGoldenSportsRequests = () => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<any>(null);
  const [result, setResult] = useState<any>(null);
  const token = useToken();
  const checkToken = useCheckToken();
  const getAllGoldenSportsRequests = async (
    ...
    try {
      setIsLoading(true);
      await axios
        .post(adminAPI.getAllRequests.goldenSports, formData, {
          headers: {
...
export default useGetAllGoldenSportsRequests;
