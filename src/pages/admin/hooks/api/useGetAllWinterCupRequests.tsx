...
const useGetAllWinterCupRequests = () => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<any>(null);
  const [result, setResult] = useState<any>(null);
  const token = useToken();
  const checkToken = useCheckToken();
  const getAllWinterCupRequests = async (
    ...
    try {
      setIsLoading(true);
      await axios
        .post(adminAPI.getAllRequests.winterCup, formData, {
          headers: {
  ...
  return {
    getAllWinterCupRequests,
    result,
    isLoading,
    error,
  };
};
export default useGetAllWinterCupRequests;
