...
const useGetAllSommerCupRequests = () => {
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<any>(null);
  const [result, setResult] = useState<any>(null);
  const token = useToken();
  const checkToken = useCheckToken();
  const getAllSommerCupRequests = async (
    ...
    try {
      setIsLoading(true);
      await axios
        .post(adminAPI.getAllRequests.sommerCup, formData, {
          headers: {
...
export default useGetAllSommerCupRequests;
